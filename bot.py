...
def execute_pending(
    ...
    # --------------------------------------------------------
    # OBTENER APERTURA REAL SOLO PARA REGISTRO
    # --------------------------------------------------------
    #
    # La ausencia temporal del stream 1M NO bloquea la orden.
    # La entrada ya está sincronizada con el reloj del servidor.
    # --------------------------------------------------------
    ...
    try:
        ...
        logger.debug(
            "%s | apertura N+1 aún no disponible: %s",
            pair,
            exc,
        )
