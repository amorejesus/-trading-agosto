...
def verify_strategy():
    ...
    print(
        "Archivo: "
        + str(getattr(strategy, "__file__", "desconocido"))
    )
...
def get_5s_candles(
    ...
    try:
        ...
        print(f"[{pair}] Error obteniendo velas: {e}")
        return None

    if not candles:
        print(f"[{pair}] API no devolvió datos.")
        return None
...
def print_analysis(pair, analysis):
    ...
    print("\n--------------------------------------")
    print(f"ANÁLISIS {pair}")
    print("--------------------------------------")

    dominant = analysis.get("dominant")

    if dominant:
        dominant_text = str(dominant).upper()
    else:
        dominant_text = "NINGUNO"

    print(f"Dominante       : {dominant_text}")
    print(
        f"Dominancia      : "
    ...
    final_control = analysis.get("final_control")

    if final_control is None:
        final_control_text = "NONE"
    else:
        final_control_text = str(final_control).upper()
...
def execute_trade(iq, pair, signal):
    ...
    try:
        ...
        print(
            f"✗ IQ Option rechazó la operación en {pair}."
        )
...
def get_trade_result(iq, order_id):
    ...
    print(
        f"Esperando resultado ({wait_seconds}s)..."
    )
...
def process_pair(iq, pair, candle_timestamp):
    ...
    if signal is None:
        print(
            f"[{pair}] ⚪ SIN OPERACIÓN"
        )
        return None

    if signal == "call":
        print(
            f"[{pair}] 🟢 SEÑAL CALL → N+1"
        )
    else:
        print(
            f"[{pair}] 🔴 SEÑAL PUT → N+1"
        )
...
def main():
    ...
    print("             MODO N+1")
    print("==========================================")
    print("MERCADO      : FOREX REAL")
    print(f"MONTO        : {AMOUNT}")
    print(f"EXPIRACIÓN   : {EXPIRATION}M")
    print(f"TIMEFRAME    : {TIMEFRAME_5S}S")
    print(f"VELAS        : {CANDLES_REQUIRED}")
    ...
    while True:
        try:
            ...
            current_timestamp = wait_for_new_5s(
                last_timestamp
            )
