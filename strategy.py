from __future__ import annotations

from typing import Any, Dict, Optional
import pandas as pd


# ============================================================
# ESTRATEGIA 1M + 12 MICROVELAS DE 5 SEGUNDOS
# ============================================================
#
# REGLA PRINCIPAL
#
# Cada vela de 1 minuto se analiza utilizando EXACTAMENTE
# sus 12 velas de 5 segundos.
#
# NO se utiliza la primera 5s para determinar la dirección.
#
# La dirección la determina el DOMINANTE MATEMÁTICO.
#
#
# COMPRADOR:
#
# BUY_SCORE =
#     suma de todos los movimientos alcistas
#     de las 12 microvelas.
#
# SELL_SCORE =
#     suma de todos los movimientos bajistas
#     de las 12 microvelas.
#
#
# DOMINANCE:
#
# abs(BUY_SCORE - SELL_SCORE)
# --------------------------------
#       BUY_SCORE + SELL_SCORE
#
#
# CALL:
#
# 1. BUY_SCORE > SELL_SCORE
# 2. DOMINANCE >= 20%
# 3. La M1 termina verde.
#
#
# PUT:
#
# 1. SELL_SCORE > BUY_SCORE
# 2. DOMINANCE >= 20%
# 3. La M1 termina roja.
#
#
# SIN SEÑAL:
#
# DOMINANCE < 20%
#
#
# IMPORTANTE:
#
# - No usa mayoría de velas.
# - No interpreta visualmente.
# - No aproxima.
# - No usa la primera 5s como dirección.
# - No mezcla microvelas entre diferentes M1.
# - Analiza exactamente 12 velas de 5s por M1.
#
# ============================================================


# ============================================================
# CONFIGURACIÓN
# ============================================================

MICRO_CANDLES_REQUIRED = 12

DOMINANCE_THRESHOLD = 0.20


# ============================================================
# UTILIDADES
# ============================================================

def _to_float(value: Any) -> Optional[float]:
    """
    Conversión segura a float.
    """

    try:
        return float(value)

    except (TypeError, ValueError):
        return None


# ============================================================
# NORMALIZAR MICROVELAS 5S
# ============================================================

def _normalize_5s(
    df: pd.DataFrame,
) -> pd.DataFrame:
    """
    Normaliza las microvelas de 5 segundos.

    Columnas obligatorias:
        open
        close

    Columnas necesarias para validar correctamente:
        high
        low

    Opcional:
        from
    """

    if df is None:
        return pd.DataFrame()

    if not isinstance(df, pd.DataFrame):
        return pd.DataFrame()

    if df.empty:
        return pd.DataFrame()

    out = df.copy()

    rename = {}

    # --------------------------------------------------------
    # Compatibilidad IQ Option
    # --------------------------------------------------------

    if "max" in out.columns and "high" not in out.columns:
        rename["max"] = "high"

    if "min" in out.columns and "low" not in out.columns:
        rename["min"] = "low"

    if "Open" in out.columns and "open" not in out.columns:
        rename["Open"] = "open"

    if "High" in out.columns and "high" not in out.columns:
        rename["High"] = "high"

    if "Low" in out.columns and "low" not in out.columns:
        rename["Low"] = "low"

    if "Close" in out.columns and "close" not in out.columns:
        rename["Close"] = "close"

    if rename:
        out.rename(
            columns=rename,
            inplace=True,
        )

    # --------------------------------------------------------
    # Validación de columnas
    # --------------------------------------------------------

    if "open" not in out.columns:
        return pd.DataFrame()

    if "close" not in out.columns:
        return pd.DataFrame()

    # --------------------------------------------------------
    # Conversión numérica
    # --------------------------------------------------------

    out["open"] = pd.to_numeric(
        out["open"],
        errors="coerce",
    )

    out["close"] = pd.to_numeric(
        out["close"],
        errors="coerce",
    )

    # --------------------------------------------------------
    # High / Low
    # --------------------------------------------------------

    if "high" in out.columns:

        out["high"] = pd.to_numeric(
            out["high"],
            errors="coerce",
        )

    if "low" in out.columns:

        out["low"] = pd.to_numeric(
            out["low"],
            errors="coerce",
        )

    # --------------------------------------------------------
    # Timestamp
    # --------------------------------------------------------

    if "from" in out.columns:

        out["from"] = pd.to_numeric(
            out["from"],
            errors="coerce",
        )

        out.dropna(
            subset=["from"],
            inplace=True,
        )

        out.sort_values(
            "from",
            inplace=True,
        )

    # --------------------------------------------------------
    # Eliminar datos inválidos
    # --------------------------------------------------------

    out.dropna(
        subset=[
            "open",
            "close",
        ],
        inplace=True,
    )

    out.reset_index(
        drop=True,
        inplace=True,
    )

    return out


# ============================================================
# VALIDAR SECUENCIA DE 5S
# ============================================================

def _validate_5s_sequence(
    micro: pd.DataFrame,
) -> bool:
    """
    Comprueba que las microvelas sean consecutivas
    cada 5 segundos.

    Si no existe timestamp, no es posible validar
    la separación temporal y se permite continuar.
    """

    if micro.empty:
        return False

    if "from" not in micro.columns:
        return True

    if len(micro) < 2:
        return False

    timestamps = (
        micro["from"]
        .astype(float)
        .tolist()
    )

    for i in range(1, len(timestamps)):

        difference = (
            timestamps[i]
            - timestamps[i - 1]
        )

        if difference != 5:
            return False

    return True


# ============================================================
# OBTENER MICROVELAS DE LA M1
# ============================================================

def _get_minute_micro_candles(
    candle_1m: pd.Series,
    candles_5s: pd.DataFrame,
) -> pd.DataFrame:
    """
    Obtiene exclusivamente las microvelas pertenecientes
    a la M1 analizada.
    """

    micro = _normalize_5s(
        candles_5s
    )

    if micro.empty:
        return pd.DataFrame()

    minute_timestamp = None

    # --------------------------------------------------------
    # Timestamp de la M1
    # --------------------------------------------------------

    if candle_1m is not None:

        try:

            if "from" in candle_1m.index:

                minute_timestamp = int(
                    float(
                        candle_1m["from"]
                    )
                )

        except (TypeError, ValueError):

            minute_timestamp = None

    # --------------------------------------------------------
    # Filtrar exactamente el minuto
    # --------------------------------------------------------

    if (
        minute_timestamp is not None
        and "from" in micro.columns
    ):

        start_time = minute_timestamp

        end_time = (
            minute_timestamp + 60
        )

        micro = micro[
            (micro["from"] >= start_time)
            &
            (micro["from"] < end_time)
        ].copy()

        micro.sort_values(
            "from",
            inplace=True,
        )

        micro.drop_duplicates(
            subset=["from"],
            keep="last",
            inplace=True,
        )

        micro.reset_index(
            drop=True,
            inplace=True,
        )

    return micro


# ============================================================
# CALCULAR DOMINANTE
# ============================================================

def _calculate_dominance(
    micro: pd.DataFrame,
) -> Dict[str, Any]:
    """
    Calcula el dominante matemático utilizando
    las 12 microvelas.

    BUY_SCORE:
        suma de movimientos alcistas.

    SELL_SCORE:
        suma de movimientos bajistas.

    Una vela:

        close > open
            movimiento comprador

        close < open
            movimiento vendedor

        close == open
            movimiento 0

    No utiliza mayoría.

    No utiliza número de velas verdes/rojas.
    """

    result: Dict[str, Any] = {

        "dominant": None,

        "buy_score": 0.0,

        "sell_score": 0.0,

        "dominance_ratio": 0.0,

        "total_movement": 0.0,
    }

    if micro.empty:
        return result

    buy_score = 0.0
    sell_score = 0.0

    # --------------------------------------------------------
    # Analizar exactamente las microvelas recibidas
    # --------------------------------------------------------

    for _, candle in micro.iterrows():

        opening = _to_float(
            candle["open"]
        )

        closing = _to_float(
            candle["close"]
        )

        if (
            opening is None
            or closing is None
        ):
            continue

        movement = (
            closing - opening
        )

        # ----------------------------------------------------
        # Movimiento comprador
        # ----------------------------------------------------

        if movement > 0:

            buy_score += movement

        # ----------------------------------------------------
        # Movimiento vendedor
        # ----------------------------------------------------

        elif movement < 0:

            sell_score += abs(
                movement
            )

    total_movement = (
        buy_score + sell_score
    )

    result["buy_score"] = buy_score
    result["sell_score"] = sell_score
    result["total_movement"] = (
        total_movement
    )

    # --------------------------------------------------------
    # No hubo movimiento
    # --------------------------------------------------------

    if total_movement <= 0:

        result["dominant"] = (
            "neutral"
        )

        result["dominance_ratio"] = (
            0.0
        )

        return result

    # --------------------------------------------------------
    # Dominancia
    # --------------------------------------------------------

    dominance_ratio = abs(
        buy_score - sell_score
    ) / total_movement

    result["dominance_ratio"] = (
        dominance_ratio
    )

    # --------------------------------------------------------
    # Comprador
    # --------------------------------------------------------

    if (
        buy_score > sell_score
        and dominance_ratio
        >= DOMINANCE_THRESHOLD
    ):

        result["dominant"] = (
            "buyer"
        )

        return result

    # --------------------------------------------------------
    # Vendedor
    # --------------------------------------------------------

    if (
        sell_score > buy_score
        and dominance_ratio
        >= DOMINANCE_THRESHOLD
    ):

        result["dominant"] = (
            "seller"
        )

        return result

    # --------------------------------------------------------
    # Equilibrio
    # --------------------------------------------------------

    result["dominant"] = (
        "neutral"
    )

    return result


# ============================================================
# ANALIZAR UNA M1
# ============================================================

def analyze_minute(
    candle_1m: pd.Series,
    candles_5s: pd.DataFrame,
) -> Dict[str, Any]:

    result: Dict[str, Any] = {

        "signal": None,

        "valid": False,

        "reason": "sin señal",

        "minute_timestamp": None,

        "minute_open": None,

        "minute_close": None,

        "micro_candles_count": 0,

        "buy_score": 0.0,

        "sell_score": 0.0,

        "dominance_ratio": 0.0,

        "dominant": None,

        "continuity_confirmed": False,
    }

    # ========================================================
    # VALIDAR M1
    # ========================================================

    if candle_1m is None:

        result["reason"] = (
            "vela de 1 minuto no disponible"
        )

        return result

    opening = _to_float(
        candle_1m.get("open")
    )

    closing = _to_float(
        candle_1m.get("close")
    )

    if opening is None:

        result["reason"] = (
            "apertura 1M inválida"
        )

        return result

    if closing is None:

        result["reason"] = (
            "cierre 1M inválido"
        )

        return result

    result["minute_open"] = opening
    result["minute_close"] = closing

    # ========================================================
    # TIMESTAMP
    # ========================================================

    if "from" in candle_1m.index:

        try:

            result[
                "minute_timestamp"
            ] = int(
                float(
                    candle_1m["from"]
                )
            )

        except (
            TypeError,
            ValueError,
        ):

            result[
                "minute_timestamp"
            ] = None

    # ========================================================
    # OBTENER MICROVELAS
    # ========================================================

    micro = _get_minute_micro_candles(
        candle_1m,
        candles_5s,
    )

    if micro.empty:

        result["reason"] = (
            "no hay microvelas 5s"
        )

        return result

    # ========================================================
    # VALIDAR SECUENCIA
    # ========================================================

    if not _validate_5s_sequence(
        micro
    ):

        result["reason"] = (
            "secuencia 5s inválida: "
            "hay huecos o timestamps incorrectos"
        )

        return result

    # ========================================================
    # EXACTAMENTE 12 MICROVELAS
    # ========================================================

    if len(micro) != MICRO_CANDLES_REQUIRED:

        result["micro_candles_count"] = (
            len(micro)
        )

        result["reason"] = (
            f"M1 inválida: se requieren "
            f"{MICRO_CANDLES_REQUIRED} velas "
            f"de 5s y se recibieron "
            f"{len(micro)}"
        )

        return result

    result["micro_candles_count"] = (
        len(micro)
    )

    # ========================================================
    # CALCULAR DOMINANTE
    # ========================================================

    dominance = _calculate_dominance(
        micro
    )

    result["buy_score"] = (
        dominance["buy_score"]
    )

    result["sell_score"] = (
        dominance["sell_score"]
    )

    result["dominance_ratio"] = (
        dominance["dominance_ratio"]
    )

    result["dominant"] = (
        dominance["dominant"]
    )

    # ========================================================
    # DOMINANTE COMPRADOR
    # ========================================================

    if dominance["dominant"] == "buyer":

        # ----------------------------------------------------
        # La M1 debe terminar verde
        # ----------------------------------------------------

        if closing <= opening:

            result["reason"] = (
                "CALL bloqueada: "
                "dominante comprador pero "
                "la M1 terminó roja o neutral"
            )

            return result

        # ----------------------------------------------------
        # CALL
        # ----------------------------------------------------

        result["signal"] = "call"

        result["valid"] = True

        result[
            "continuity_confirmed"
        ] = True

        result["reason"] = (
            "CALL confirmada: "
            "dominante comprador + "
            "M1 verde"
        )

        return result

    # ========================================================
    # DOMINANTE VENDEDOR
    # ========================================================

    if dominance["dominant"] == "seller":

        # ----------------------------------------------------
        # La M1 debe terminar roja
        # ----------------------------------------------------

        if closing >= opening:

            result["reason"] = (
                "PUT bloqueada: "
                "dominante vendedor pero "
                "la M1 terminó verde o neutral"
            )

            return result

        # ----------------------------------------------------
        # PUT
        # ----------------------------------------------------

        result["signal"] = "put"

        result["valid"] = True

        result[
            "continuity_confirmed"
        ] = True

        result["reason"] = (
            "PUT confirmada: "
            "dominante vendedor + "
            "M1 roja"
        )

        return result

    # ========================================================
    # NEUTRAL
    # ========================================================

    result["reason"] = (
        "sin señal: "
        "no existe dominante suficiente"
    )

    return result


# ============================================================
# API PRINCIPAL
# ============================================================

def analyze_market(
    candle_1m: pd.Series,
    candles_5s: pd.DataFrame,
) -> Dict[str, Any]:

    return analyze_minute(
        candle_1m,
        candles_5s,
    )


# ============================================================
# API SIMPLE
# ============================================================

def get_signal(
    candle_1m: pd.Series,
    candles_5s: pd.DataFrame,
) -> Optional[str]:

    result = analyze_market(
        candle_1m,
        candles_5s,
    )

    return result.get(
        "signal"
    )


# ============================================================
# COMPATIBILIDAD
# ============================================================

def signal(
    candle_1m: pd.Series,
    candles_5s: pd.DataFrame,
) -> Optional[str]:

    return get_signal(
        candle_1m,
        candles_5s,
    )


# ============================================================
# PRUEBA
# ============================================================

if __name__ == "__main__":

    print(
        "strategy.py cargado correctamente."
    )

    print(
        "Estrategia: 1M + 12 microvelas de 5S"
    )

    print(
        "Dirección: dominante matemático."
    )

    print(
        "Umbral de dominancia: "
        f"{DOMINANCE_THRESHOLD:.0%}"
    )

    print(
        "CALL = dominante comprador + M1 verde"
    )

    print(
        "PUT = dominante vendedor + M1 roja"
    )
